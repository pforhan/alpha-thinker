...
- [ ] **Web runtime correctness: Kotlin/Wasm (`wasmJs`), not Compose HTML.** The previous entry uses the Compose-HTML DOM runtime (`renderComposableInBody`, `org.jetbrains.compose.html`), which shares only the Compose *runtime* and cannot render the shared foundation/Material3 UI (`Surface`, `LazyColumn`, `MaterialTheme`…). Verify in a browser that the app actually draws; if (as expected) it renders a blank DOM tree, register the **`wasmJs`** target in `shared` — mirroring the `jsMain` Room-free actuals (`WebPlatformContext`, in-memory `provideStorage`, `NavApp`, `now`/`randomUUID`) into `wasmJsMain` — and convert the web entry to the supported Canvas path with a Skia backend plus the current "Compose for Web" template (`index.html`/wasm bootstrap), keeping the same `App(WebPlatformContext())` call. Then exercise dev and production web runs so the shared Material UI is confirmed in-browser. Proof: `./gradlew :webApp:jsBrowserDistribution` completes and a browser visit renders the app. *(Deferred note: `InMemoryStorage` doesn't survive page reloads — back it with `localStorage` afterwards and add a web-persistence test.)*
