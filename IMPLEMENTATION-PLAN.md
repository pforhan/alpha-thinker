...
- [ ] **Pull Room persistence out of `androidMain` into a shareable KMP module:** the whole Room stack is trapped in `androidMain` — `AppDatabase`, `Entities.kt`, `ProjectDao.kt`, `RoomStorage.kt`, and the `DatabaseUtils` actuals — so Android is the only target with real persistence (web is in-memory, desktop/iOS would follow suit). Room KMP (the `2.7.0-alpha11` already on `room`) supports Android, JVM, and iOS native, so split an "api" boundary — the target-agnostic `Storage` interface + models + `PlatformContext` that live in `shared` commonMain — from a dedicated `room` module (or `roomMain` source set) holding the entities, DAOs, `AppDatabase`, `RoomStorage`, and mappers against the KMP Room artifacts. `shared` then pulls the room module into every target that can host SQLite (Android, desktop/JVM, iOS); web keeps its in-memory actual since Room doesn't ship for `wasmJs`. Each target supplies the small `AppDatabaseConstructor` / `provideDatabaseBuilder` actuals (Android via `AndroidPlatformContext.context`, iOS/JVM via their file-path/driver APIs) — in the room module the `expect` can live in `commonMain` with the KSP-generated constructor actual per target, which is what the Baseline step couldn't do while everything sat in `androidMain`. Proof: `./gradlew :shared:allTests :androidApp:assembleDebug` stays green after the split, with `RoomStorage` compiling against KMP Room instead of `androidMain`-only artifacts.
- [ ] **Set up Compose Multiplatform Desktop (macOS / JVM) target:** add `jvm()` to `shared` plus a desktop entry (`compose.desktop.currentOs`, `application` plugin). `desktopMain` is currently a stub — `util/Utils.kt` has only `randomUUID()` and no `now()` actual, and `NavApp` / `PlatformContext` / `provideStorage` actuals are missing — so add them and a `main()`. Wire `provideStorage` to the Step-5 Room module so desktop gets real persistence, not in-memory. Proof: `./gradlew :desktopApp:run` opens the app window.
- [ ] **Set up Compose Multiplatform iOS target:** add `iosArm64` + `iosSimulatorArm64` targets and the iOS actuals (`NavApp`, `PlatformContext`/`provideStorage`, `now()`/`randomUUID()`); pull in the Step-5 Room module for on-device persistence; runnable via an Xcode-hosted app project. Proof: `./gradlew :shared:compileKotlinIosArm64 :shared:compileKotlinIosSimulatorArm64` succeed.
...
- [ ] Rework saving answers Let's make the answer dialog have a close button (or tap off the dialog) and a completed toggle or checkbox that moves things from draft to answered.  Clean up all other button placements
